...
0. **先确认拓扑** → 目标设备是同网段直连，还是要经路由器？跨网段/访问路由器下挂设备不通，先看 §0，别直接在 Windows 防火墙上找原因
...
## 0. 先确认拓扑：同网段直连，还是经路由器

**同网段直连是纯 L2、不经过路由器**；跨网段则必经路由器。两条路径，故障原因完全不同——
后者不通时先怀疑拓扑与路由器策略，别在 Windows 防火墙上空转（防火墙排查见 §1）。

**一条实测限制**：域主机**只有在 2 网段**时，才能经路由器访问到**该路由器下挂的设备**；
不在同一网段时这条路径不通。

- 处置方向：**在路由器后台改设置**（路由/转发/隔离相关项）。
- **未验证的备选**：把域主机的**静态 IP 改到 2 网段** —— **尚未实测**，
  仅作待试方向记录，**不要当成已验证结论向用户断言**。
  真要试属**高风险**：域机器换 IP 会牵连 AD/DNS 注册、许可服务器绑定、共享盘映射、
  以及已有的 VNC/RDP 目标地址（连不上就得改回来）；主机是 headless，
  **一旦配错只能到机器前处理**。务必在 VNC/物理控制台可达时改，改完先确认能连回来。
