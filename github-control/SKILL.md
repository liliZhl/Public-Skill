...
**往两仓新增一个技能**（别用 `--all`，它会把本机所有实验性技能一起带进去）：

1. 先把技能目录复制进**私有仓工作副本** —— 只为让发现阶段"看见"它，
   内容随后会被源目录整体覆盖，所以这一步不需要先脱敏：
   `cp -r ~/.workbuddy/skills/<name> ~/repos/Private-Skill/<name>`
2. 跑一次默认同步（**不带 `--push`**），确认技能数正确、且独立复查无残留
3. 再 `--push`
4. 两仓根 `README.md` 的**技能清单、目录树、安装命令**要手工补（同步工具只管技能目录，
   不碰 README），公开仓还要把新引入的占位符补进"占位符对照表"
