...
- **误报很常见**：显卡型号 `<ACCOUNT>` 会命中"域账号样式"、
  `es.cursor = sel` 里的 `cursor` 会命中"AI 工具目录"。要调出原文看。
- **漏报更危险**：所以工具里宁可放"通用形状"的模式（如
  `[A-Za-z]:\\Users\\<名字>`），也不依赖已知的具体值。
- 代码类风险（凭据）好扫；**措辞类风险要靠眼睛**：
  `docs/` 里的「交付物 / 交接文档 / 开发机 / 目标机」这类措辞，
